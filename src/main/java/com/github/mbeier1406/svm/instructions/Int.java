...
public class Int extends InstructionBase implements InstructionInterface<Short>, IntInterface<Short> {
	...
	/**
	 * Entnimmt dem im Parameter übergebenen Byte den Codes des Moduls, das die
	 * weitere Programmausführung übernimmt (siehe {@linkplain IntInterface#MODULES}
	 * und sucht in der Map nach dem in {@code Register 1} angegeben Service und
	 * ruft dessen {@code execute()}-Funktion mit den {@code Registern 2-4} als Parameter auf.
	 * <p/>
	 * {@inheritDoc}
	...
	public int execute(byte[] params) throws SVMException {
		...
		Short function = this.alu.getRegisterValue(1); // Register 1 codiert die Funktion, die aufgerufen werden soll
		...
		var param1 = this.alu.getRegisterValue(2); // Register 2 enthält immer den ersten Parameter der Servicefunktion
		var param2 = this.alu.getRegisterValue(3); // Register 3 enthält immer den ersten Parameter der Servicefunktion
		var param3 = this.alu.getRegisterValue(4); // Register 4 enthält immer den ersten Parameter der Servicefunktion
		LOGGER.trace("INT: param1='{}'; param2='{}'; param3='{}'", param1, param2, param3);
		Object returnCode;
		try {
			returnCode = execute.invoke(param1, param2, param3);
